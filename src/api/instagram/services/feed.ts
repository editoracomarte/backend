...
/**
 * Resolves the Instagram feed for the home page. Fresh cache wins; otherwise it
 * hits the RapidAPI endpoint and refreshes the cache; on any failure it falls
 * back to the posts entered manually in the single type.
 *
 * Lives in its own file (not the factory-generated `instagram.ts`) so it is
 * instrumented for coverage — the generated service just delegates here.
 */
export async function getPosts(strapi: Core.Strapi): Promise<Post[]> {
  ...
  return getManualPosts(strapi);
}

export async function fetchFromRapidApi(): Promise<Post[]> {
...
export async function getManualPosts(strapi: Core.Strapi): Promise<Post[]> {
